"""
test_FTM.py  (ocean acoustic edition)
-------------------------------------
Evaluate a trained FTM checkpoint on the 2-channel ocean-acoustic complex
pressure dataset (channels = [real, imag]).

It rebuilds the *shared* continuous spatial basis  f_x(x), f_y(y)  and combines
it with the per-sample Tucker cores  G(omega)  to reconstruct the full field

        p_hat(x, y, omega) = vec(G_omega)^T ( f_x(x) ⊗ f_y(y) ),

then measures the relative reconstruction error and visualises the result as
real / imag / |p| / phase  (GT vs Pred vs Err).

Note: FTM is transductive — cores exist only for the samples seen during FTM
training, so this evaluates reconstruction quality on those samples (use the
same dataset the checkpoint was trained on).  Only min(B_ckpt, B_data) samples
are evaluated.

Outputs
-------
1) Per-channel error curves (frequency RMSE) + per-sample RMSE curve
2) Reconstruction vs ground-truth comparison figures (re / im / |p| / phase)
3) Metric CSV tables and summary JSON

Usage
-----
    python test_FTM.py --ckpt ckp/ftm_ocean.pt --data_h5 ocean_dataset.h5
"""
...
def _ang_err(pred_ph: np.ndarray, gt_ph: np.ndarray) -> np.ndarray:
    """Wrapped absolute phase error in [0, pi]."""
    d = pred_ph - gt_ph
...
def _plot_compare_2ch(
    ...
    """Comparison grid: real / imag / |p| / phase  ×  GT | Pred | |Err|.

    Display matches Visualize_dataset.py — depth↓ (rows) vs range→ (cols),
    Re/Im on a symmetric 98th-percentile scale (acoustic fields have a strong
    near-source spike), |p| shared linear scale, phase cyclic (twilight)."""
    gt_re, gt_im = gt[..., 0],   gt[..., 1]
    pd_re, pd_im = pred[..., 0], pred[..., 1]
    gt_amp, pd_amp = np.abs(gt_re + 1j * gt_im), np.abs(pd_re + 1j * pd_im)
    gt_ph, pd_ph   = np.angle(gt_re + 1j * gt_im), np.angle(pd_re + 1j * pd_im)

    # robust symmetric limit for re/im (shared GT+Pred), shared linear for |p|
    a_re = float(np.percentile(np.abs(np.concatenate([gt_re, pd_re])), 98)) or 1.0
    a_im = float(np.percentile(np.abs(np.concatenate([gt_im, pd_im])), 98)) or 1.0
    amp_max = float(max(gt_amp.max(), pd_amp.max())) or 1.0

    # rows: (label, gt, pred, err, cmap, vmin, vmax, err_cmap, err_vmax)
    rows = [
        ("Real",  gt_re,  pd_re,  np.abs(pd_re - gt_re),  "RdBu_r",   -a_re, a_re,  "magma", None),
        ("Imag",  gt_im,  pd_im,  np.abs(pd_im - gt_im),  "RdBu_r",   -a_im, a_im,  "magma", None),
        ("|p|",   gt_amp, pd_amp, np.abs(pd_amp - gt_amp),"magma",     0.0,  amp_max,"magma", None),
        ("phase", gt_ph,  pd_ph,  _ang_err(pd_ph, gt_ph), "twilight", -np.pi, np.pi,"magma", np.pi),
    ]

    fig, axes = plt.subplots(4, 3, figsize=(11, 13))
    for r, (label, g, p, err, cmap, vmin, vmax, ecmap, evmax) in enumerate(rows):
        for c, (img, ttl, is_err) in enumerate(
            [(g, f"GT {label}", False), (p, f"Pred {label}", False), (err, f"|Err| {label}", True)]
        ):
            ax = axes[r, c]
            if is_err:
                kw = {"cmap": ecmap, "vmin": 0.0}
                if evmax is not None:
                    kw["vmax"] = evmax
            else:
                kw = {"cmap": cmap, "vmin": vmin, "vmax": vmax}
            im = ax.imshow(img, origin="upper", aspect="auto", **kw)
            ax.set_title(ttl, fontsize=9)
            ax.set_xticks([]); ax.set_yticks([])
            fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)

    title = f"sample={sample_idx}  freq_idx={freq_idx}  ω={omega_value:.1f} Hz  (depth↓ × range→)"
    if ch_prefix:
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    with h5py.File(args.data_h5, "r") as f:
        ...
        flat_case = case_rel.reshape(-1)   # case_rel is (B_eval, M_data), row-major
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Evaluate ocean-acoustic FTM checkpoint (C=2: real, imag)"
    )
    p.add_argument("--ckpt",              type=str, default="ckp/ftm_gpu_checkpoint_fixedmedium.pt")
    p.add_argument("--data_h5",           type=str, default="ocean_dataset_fixedmedium.h5")
    p.add_argument("--out_dir",           type=str, default="visual_data/ftm_eval")

    p.add_argument("--batch_size",        type=int, default=32)
    p.add_argument("--max_eval_samples",  type=int, default=0,
                   help="0 = all available (= min(B_ckpt, B_data)).")
