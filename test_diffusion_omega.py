"""
test_diffusion_omega.py
-----------------------
Evaluate conditional U-Net diffusion + DPS for frequency-domain field reconstruction
with omega-conditioned FTM basis decoding.

Pipeline
--------
1) Load trained diffusion model p(G | omega) on 2-channel core images.
2) Load omega-conditioned FTM basis networks (net_x/net_y) from train_FTM_omega.py checkpoint.
3) For each test case (sample, frequency), build Phi_omega and run:
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    with h5py.File(data_h5_path, "r") as f:
        ...
        for i_case, (b_idx, m_idx) in enumerate(cases, start=1):
            ...
            omega_case_t = torch.tensor([[omega_cond]], device=device, dtype=torch.float32)
            with torch.no_grad():
                phi_case_t = build_phi_conditional(ftm["net_x"], ftm["net_y"], x_t, y_t, omega_case_t)[0]
            phi_np = phi_case_t.detach().cpu().numpy().astype(np.float32)
            if phi_np.shape[1] != r:
                raise ValueError(f"Phi rank mismatch: phi={phi_np.shape[1]}, expected={r}")
            phi_t_full = phi_case_t
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Test omega-conditioned diffusion reconstruction with DPS")

    p.add_argument("--diff_ckpt", type=str, default="ckp/diffusion_core_omega.pt")
    ...
    p.add_argument("--out_dir", type=str, default="visual_data/diffusion_omega_eval_msk10")
    ...
    p.add_argument("--dps_weight", type=float, default=1)
