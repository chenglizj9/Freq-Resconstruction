...
test_diffusion.py
-----------------
Evaluate conditional U-Net diffusion + DPS for frequency-domain field reconstruction.
    ...
    for i, t_idx in enumerate(timestep_seq):
        ...
        if use_dps or use_phys:
            ...
            if use_dps:
                ...
                # diff_re = pred_re_obs - y_re
                # diff_im = pred_im_obs - y_im
                # norm_re = diff_re / (torch.abs(y_re) + eps)
                # norm_im = diff_im / (torch.abs(y_im) + eps)
                # like_loss = torch.mean(norm_re ** 2 + norm_im ** 2)
                ...
                # like_loss = torch.clamp(like_loss, max=1.0)
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    with h5py.File(data_h5_path, "r") as f:
        ...
        solver: Optional[HarmonicHeatSolver] = None

        if use_phys_guidance:
            if h != w:
                print("[WARN] phys_weight>0 but non-square grid, physics guidance is disabled.")
            else:
                l_val = float(h5_meta.get("L", args.heat_L))
                diffusivity_val = float(h5_meta.get("diffusivity", args.heat_diffusivity))
                capacity_val = float(h5_meta.get("capacity", args.heat_capacity))
                solver = HarmonicHeatSolver(
                    N=h,
                    L=l_val,
                    diffusivity=diffusivity_val,
                ...
                physics_ready = True
                print(
                    f"[INFO] physics guidance enabled: L={l_val:.4g}, diffusivity={diffusivity_val:.4g}, "
                    f"capacity={capacity_val:.4g}, source_sigma={source_sigma:.4g}, data_scale={data_scale:.4g}"
                )
        ...
        for i_case, (b_idx, m_idx) in enumerate(cases, start=1):
            ...
            core_gt_lstsq = _solve_lstsq_core_from_gt(
                gt=gt,
                phi_np=phi_np,
                rx=rx,
                ry=ry,
            ...
            try:
                pde_res_prior = evaluate_physics_residual(
                    pred=pred_prior,
                    h5_file=f,
                    sample_idx=b_idx,
                    omega=omega_val,
                ...
                pde_res_prior = np.nan

            try:
                pde_res_dps = evaluate_physics_residual(
                    pred=pred_dps,
                    h5_file=f,
                    sample_idx=b_idx,
                    omega=omega_val,
            ...
            rows.append(
                {
                    ...
                    "pde_res_prior": pde_res_prior,
                    "pde_res_dps": pde_res_dps,
                }
            )
    ...
    with open(csv_path, "w", encoding="utf-8") as f:
        ...
        for r_ in rows_sorted:
            f.write(
                ...
                f"{r_['pde_res_prior']:.8g},{r_['pde_res_dps']:.8g}\n"
            )
    ...
    mean_pde_prior = float(np.nanmean([r_["pde_res_prior"] for r_ in rows_sorted]))
    mean_pde_dps = float(np.nanmean([r_["pde_res_dps"] for r_ in rows_sorted]))
    ...
    summary = {
        ...
        "mean_pde_res_prior": mean_pde_prior,
        "mean_pde_res_dps": mean_pde_dps,
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Test conditional diffusion reconstruction with DPS")

    p.add_argument("--diff_ckpt", type=str, default="heat_data/ckp/diffusion_core.pt")
    ...
    p.add_argument("--out_dir", type=str, default="heat_data/visual_data/diffusion_eval")
    ...
    p.add_argument("--heat_L", type=float, default=1.0)
    p.add_argument("--heat_diffusivity", type=float, default=1.0)
    p.add_argument("--heat_capacity", type=float, default=1.0)
    p.add_argument("--source_sigma", type=float, default=0.04)

    p.add_argument("--num_visualize", type=int, default=105)
