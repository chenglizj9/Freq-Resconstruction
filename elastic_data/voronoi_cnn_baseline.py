"""
voronoi_cnn_baseline.py  (elastic wave edition)
------------------------------------------------
Voronoi-CNN baseline for 4-channel elastic field reconstruction.
...
Input channels (9):
  voronoi_ux_re, voronoi_ux_im,  (nearest-neighbor filled)
  voronoi_uy_re, voronoi_uy_im,
  mask, omega_norm, x_coord, y_coord   →  8 channels total
...
Train:
    python voronoi_cnn_baseline.py --mode train \\
        --train_h5 elastic_dataset.h5 --out ckp/voronoi_cnn_elastic.pt
Eval:
    python voronoi_cnn_baseline.py --mode eval \\
        --ckpt ckp/voronoi_cnn_elastic.pt \\
...
def voronoi_fill(field: np.ndarray, mask: np.ndarray) -> np.ndarray:
    ...
    obs_yx   = np.argwhere(obs_mask)              # (K, 2)
    if len(obs_yx) == 0:
        return np.zeros_like(field)
    obs_vals = field[obs_mask]                    # (K, C)
    rows, cols = np.meshgrid(np.arange(H), np.arange(W), indexing="ij")
    all_pts = np.stack([rows.ravel(), cols.ravel()], axis=-1)
    tree = cKDTree(obs_yx)
    _, idx = tree.query(all_pts)
    return obs_vals[idx].reshape(H, W, C).astype(np.float32)
...
class ElasticVoronoiData(Dataset):
    def __init__(
        self,
        h5_path: str | Path,
        ...
        with h5py.File(self.path, "r") as f:
            ...
            if "data" in f:
                ...
                raise KeyError("HDF5 must contain 'data'")
    ...
    def __getitem__(self, idx):
        ...
        voronoi = voronoi_fill(field, mask)            # (H, W, 4)
        x = np.stack(
            ...
            axis=0).astype(np.float32)                 # (C+4, H, W)
...
class VoronoiCNN(nn.Module):
    def __init__(self, in_channels: int = 8, out_channels: int = 4, base_channels: int = 64):
        super().__init__()
...
def train_model(args):
    ...
    tr_ds = ElasticVoronoiData(args.train_h5, tr_ids, args.train_freq_indices)
    va_ds = ElasticVoronoiData(args.train_h5, va_ids, args.eval_freq_indices)
    ...
    print(f"\nVoronoi-CNN (elastic) | in={in_ch} out={out_ch} base={args.base_channels} device={device}")
...
def _plot_eval(out_path, gt, pred, mask, s, f, omega, rmse, obs_r, unobs_r, dpi=150):
    C = gt.shape[-1]
    fig, axes = plt.subplots(C, 3, figsize=(15, 5*C))
    labels = ["ux_re","ux_im","uy_re","uy_im"][:C]
    for ci in range(C):
        row = axes[ci] if C > 1 else axes
        for ax, (img, title) in zip(row, [
            (gt[...,ci], f"GT {labels[ci]}"),
            (pred[...,ci], f"Pred {labels[ci]}"),
            (np.abs(pred[...,ci]-gt[...,ci]), f"Err {labels[ci]}"),
        ]):
            im = ax.imshow(img, origin="lower", cmap="viridis" if "Err" not in title else "magma")
            ax.set_title(title); ax.set_xticks([]); ax.set_yticks([])
            fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    fig.suptitle(f"s={s} f={f} ω={omega:.3f} | rmse={rmse:.2e} obs={obs_r:.2e} unobs={unobs_r:.2e}", y=0.995)
...
def evaluate_model(args):
    ...
    ds   = ElasticVoronoiData(data_path, list(range(max_s)), args.eval_freq_indices)
            ...
            for i in range(pred.shape[0]):
                ...
                if vis_cnt < args.num_visualize:
                    ...
                    _plot_eval(vp, gt_i, pred_i, mask_i, int(s_idx[i]), int(f_idx[i]),
                               float(omega_r[i]), rmse, obs_r, unobs_r, args.vis_dpi)
...
def build_parser():
    p = argparse.ArgumentParser("Voronoi-CNN baseline — elastic wave")
    ...
    p.add_argument("--ckpt",     default="ckp/voronoi_cnn_elastic.pt")
    p.add_argument("--out",      default="ckp/voronoi_cnn_elastic.pt")
    p.add_argument("--out_dir",  default="visual_data/voronoi_cnn_elastic_eval_msk0.01")
